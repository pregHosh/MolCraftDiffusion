...
- **Build locally** — ditmc, equifm, nextmol, pmdm, and diffsmol's converted
  upstream checkpoint. Weights exist,
...
no loss, no dataset, no training config, and no `tasks.task_type` of its own, just
an `interference.task_type: silvr`. It runs on **any** EDM checkpoint, so use a
pretrained one from the zoo rather than training anything:

```bash
MolCraftDiff zoo fetch edm/geom      # the GEOM-drugs prior SILVR was built on
...
| Task config | `task_type` | Conditioned on | Notes |
| :--- | :--- | :--- | :--- |
| `diffusion_diffsmol.yaml` | `diffusion_diffsmol` | Molecular **shape** | **Shape-matching / bioisostere design** — hand it a reference molecule's shape envelope and it generates new molecules that occupy it. Shape conditioning is available and working: fetch the `moses2_shape` variant and run `examples/diffsmol_moses2_shape_generate.yaml`. Two things to set: keep `guide_stren` at **0.0** (that is the shape-conditioned setting; the higher values inherited from upstream's sampler degrade validity badly), and give it a reference envelope built by the bundled precompute script. Heavy atoms only, no bonds — the chemistry is perceived afterwards. ShEPhERD below is the alternative when you also need electrostatics and pharmacophores. |
