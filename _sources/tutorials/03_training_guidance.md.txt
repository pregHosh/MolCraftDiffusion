...
We use the exact same override-only configuration workflow introduced in **[Tutorial 1: Training a Diffusion Model](01_training_diffusion.md)**, but we load the `guidance` tasks and `regression` trainer templates via the `defaults` list.

```yaml
# Inside my_guidance_run.yaml
defaults:
  - data: mol_dataset
...
| `data.ase_db_path` | `data: {ase_db_path: "data/my_dataset.db"}` | Path to your compiled ASE database. |
...
Here is a complete `my_guidance_run.yaml` example:
